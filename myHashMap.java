...
class myHashMap<K, V> {
    ...
    public V remove(K key) {
        // get bucket index to loop through
        int index = getBucketIndex(key);
        // create the head node to loop
        HashNode<K, V> head = bucket.get(index);
        // set prev to null which will help to replace
        ...
        while (head != null) {
            // if we find the key:
            if (head.key.equals(key)) {
                // if prev is null then we can set index to head.next instead of moving other
                // pointers
                if (prev == null) {
                    bucket.set(index, head.next);
                } else {
                    // otherwise we want to set the previous to head.next (removing node)
                    prev.next = head.next;
                }
                // decrease size
                size--;
        ...
        return null; // return null if not found or head == null;
    }
